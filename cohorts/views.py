...
class BulkAttendanceView(APIView):
    ...
    def post(self, request, session_id):
        ...
        # First attendance submission for this session = official class start time.
        if not session.started_at:
            session.started_at = timezone.now()
            session.start_latitude = request.data.get('latitude') or None
            session.start_longitude = request.data.get('longitude') or None
            session.save(update_fields=['started_at', 'start_latitude', 'start_longitude'])
